...
sim_dat <- function(n_sim, study_dat_t) { # what is called simulation here is actually bootstrapping
  ...
  if (has_na) {
    ...
    simvec_ord <- 1:(nsub * nvis)
  }
  # holders for sim results
  sim_arrays <- matrix(
    data = rep(0, (nvis + 2) * nsub * n_sim), nrow = nsub * n_sim, ncol = nvis + 2,
  ...
  for (ii in 1:length(gv_sites)) { # ii <- 0;   ii <- ii+1
    ...
    for (jj in 1:n_sigs) { # jj <- 0;     jj <- jj+1
      ...
      writeDataTable(wb.res,
        sheet = ws_n, x = as.data.frame(pattern1), startCol = 1, startRow = nextrow,
      ...
      nextrow <- nextrow + 1 # ?
      writeDataTable(wb.res,
        sheet = ws_n, x = as.data.frame(pattern2), startCol = 1, startRow = nextrow,
        colNames = TRUE, rowNames = FALSE, withFilter = FALSE
      )
      ...
      nextrow <- nextrow + 1 # ?
      writeDataTable(wb.res,
        sheet = ws_n, x = as.data.frame(pattern3), startCol = 1, startRow = nextrow,
        colNames = TRUE, rowNames = FALSE, withFilter = FALSE
      )
      ...
      nextrow <- nextrow + 1 # ?
      writeDataTable(wb.res,
        sheet = ws_n, x = as.data.frame(pattern4), startCol = 1, startRow = nextrow,
        colNames = TRUE, rowNames = FALSE, withFilter = FALSE
      )
    } # for jj
  } # for ii
